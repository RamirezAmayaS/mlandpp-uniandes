...
def kmeans(puntos, k):
    ...
    Process = Iteratively assign the coordinates to a certain mean. After all points are assigned, the clustering means are reestimated based on the points assigned to each of these means.
    """

    eps = 0 # Tolerance for the while 

    # Initialization of variables
    centroides = np.random.randint(np.min(puntos), np.max(puntos), size=(k, 2)) # Initial means
